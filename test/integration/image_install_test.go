...
const (
	imgLocalTag    = "mgtt-provider-docker-it:test"
	providerName   = "docker"
	expectCaps     = "docker"
	expectNetwork  = ""
	registryPort   = "15804"
	registryName   = "mgtt-provider-docker-it-registry"
	localPushTag   = "localhost:15804/mgtt-provider-docker:it"
)
...
func TestImageInstall_Capabilities(t *testing.T) {
	...
	digest := fetchManifestDigest(t, registryPort, "mgtt-provider-docker", "it")
	if digest == "" {
		t.Fatal("could not resolve manifest digest after push")
	}
	digestRef := fmt.Sprintf("localhost:%s/mgtt-provider-docker@%s", registryPort, digest)
